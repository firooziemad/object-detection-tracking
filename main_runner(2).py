    ...
    while True:
        if not paused:
            ...
            if history_redetect and tracking_success and box:
                ...
                if history_check_counter >= int(fps):
                    history_check_counter = 0
        ...
        info_y += 20
        cv2.putText(display_frame, f"FPS: {display_fps}", (10, info_y), cv2.FONT_HERSHEY_SIMPLEX, 0.6, (255, 255, 255), 1)
        info_y += 20
        if auto_redetect:
        ...
        elif key == ord('l'): limit_fps = not limit_fps
...
git commit -m "feat: Add FPS counter and framerate limiting toggle"